...
<p align="center">
  <img src="assets/sistemevaluasiakademik.png" width="600">
</p>
...
* `nilai_tugas` → Nilai tugas harian/praktikum
* `nilai_uts` → Nilai Ujian Tengah Semester
* `nilai_uas` → Nilai Ujian Akhir Semester

Struktur ini memungkinkan pengelompokan beberapa properti mahasiswa di dalam satu item array.
...
Fungsi ini digunakan untuk menghitung nilai akhir dengan bobot penilaian:

* Tugas = 30%
* UTS = 30%
* UAS = 40%

Fungsi menerima 3 parameter input yaitu `$tugas`, `$uts`, dan `$uas`, kemudian mengembalikan hasil kalkulasi aritmatika nilai akhir.
...
Fungsi ini mengevaluasi nilai akhir menjadi **grade huruf** menggunakan percabangan `if/elseif/else`:
...
Fungsi ini membandingkan nilai akhir mahasiswa dengan KKM kelulusan (nilai 60) menggunakan operator perbandingan `>=` untuk menentukan status kelulusan.

---

### 5. Loop Pemrosesan Data & Statistik
...
Melalui perulangan `foreach`, setiap data mahasiswa dihitung nilai akhir, grade, dan statusnya secara otomatis. Pada loop ini juga dihitung akumulasi total nilai dan pencarian nilai tertinggi untuk statistik kelas.

---

### 6. Desain Visual Premium dengan CSS Vanilla

Halaman web tidak menggunakan framework CSS eksternal seperti Bootstrap secara default, melainkan menggunakan kode **Vanilla CSS** yang didesain secara manual untuk menghasilkan visual premium bergaya *Dark Mode* dengan gradien radial yang memukau, efek *glassmorphism* (`backdrop-filter`), bayangan lembut (*box shadow*), tata letak grid responsif, serta lencana warna (*badge*) yang dinamis untuk visualisasi grade dan status kelulusan.

---

### 7. Perhitungan Statistik Kelas (Rata-rata & Tertinggi)

```php
$rataRataKelas = count($mahasiswaList) > 0 ? $totalNilaiAkhir / count($mahasiswaList) : 0;
```

Statistik kelas dihitung di bagian backend PHP dan ditampilkan dalam bentuk kartu dashboard informatif di bagian atas tabel:
* **Rata-rata Kelas**: Diperoleh dari pembagian total nilai akhir dengan jumlah mahasiswa.
* **Nilai Tertinggi**: Ditentukan dengan perbandingan dinamis di dalam perulangan dan menampilkan nama mahasiswa yang bersangkutan.

---

## Kesimpulan

Program Sistem Penilaian Mahasiswa ini berhasil diimplementasikan dengan memanfaatkan konsep pemrograman web dasar menggunakan PHP:
* Struktur data yang rapi menggunakan array asosiatif multidimensi.
* Pembagian kode fungsional menggunakan fungsi modular (`hitungNilaiAkhir`, `tentukanGrade`, `tentukanStatus`).
* Implementasi operator aritmatika dan perbandingan untuk pemrosesan nilai.
* Tampilan antarmuka berbasis web responsif yang modern dengan integrasi CSS kustom.

---

## Referensi

* Dokumentasi Resmi PHP: [php.net](https://www.php.net/docs.php)
